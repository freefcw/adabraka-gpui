...
pub struct PinchEvent {
    ...
    /// The zoom delta for this event. Positive values indicate zooming in,
    /// negative values indicate zooming out. Native pinch gestures report
    /// the change in scale since the previous event. Wheel-based synthetic
    /// pinch gestures report the normalized wheel-notch delta.
    pub delta: f32,
...
mod test {
    ...
    fn test_multi_modifier_gesture_does_not_dispatch_standalone_modifier_binding(
        ...
        test_view.update_in(cx, |test_view, window, _cx| {
            window.focus(&test_view.focus_handle);
        });
