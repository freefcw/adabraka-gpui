...
use super::{HiLoWord, current_modifiers, logical_point};
use crate::*;

/// Default viewport size in pixels. The actual content size doesn't matter because the viewport
/// is only used for gesture recognition, not visual output.
...
impl DirectManipulationHandler {
    pub(crate) fn new(window: HWND, scale_factor: f32) -> Result<Self> {
        unsafe {
    ...
    pub(crate) fn on_pointer_hit_test(&self, wparam: WPARAM) {
        unsafe {
...
impl IDirectManipulationViewportEventHandler_Impl for DirectManipulationEventHandler_Impl {
    fn OnViewportStatusChanged(
        ...
        if current == DIRECTMANIPULATION_READY {
            ...
            // ZoomToRect triggers another RUNNING -> READY cycle, so avoid an infinite loop.
    ...
    fn OnContentUpdated(
        ...
        // Classify each gesture as scroll or pinch and only emit one GPUI event stream.
