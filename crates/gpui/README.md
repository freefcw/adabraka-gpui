...
Internal core crate for [Adabraka GPUI](https://crates.io/crates/adabraka-gpui).

This package contains GPUI's application state, element system, rendering contracts, test runtime, and the desktop backends that are being extracted during the upstream-aligned crate migration. Most applications should depend on `adabraka-gpui`, which preserves the stable `gpui` crate name and constructs the appropriate platform backend.
