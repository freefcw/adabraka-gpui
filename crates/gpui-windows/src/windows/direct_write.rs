    ...
    unsafe fn get_font_id_from_font_collection(
        ...
        let font = unsafe {
            fontset
                .GetMatchingFonts(
                    &HSTRING::from(family_name),
                    directwrite_font_weight(font_weight),
                    DWRITE_FONT_STRETCH_NORMAL,
                    directwrite_font_style(font_style),
                )
                .log_err()?
        };
    ...
    fn layout_line(
        ...
        unsafe {
            ...
            let text_layout = {
                ...
                let cache_key = (first_run.font_id.0, font_size_value.to_bits());
            ...
            for run in font_runs {
                ...
                let font_scale = f32::from(font_size) / metrics.Base.designUnitsPerEm as f32;
                ...
                text_layout.SetFontSize(f32::from(font_size), text_range)?;
...
fn directwrite_font_style(style: FontStyle) -> DWRITE_FONT_STYLE {
    match style {
...
fn directwrite_font_weight(weight: FontWeight) -> DWRITE_FONT_WEIGHT {
...
fn get_font_identifier_and_font_struct(
    ...
    let font_struct = Font {
        ...
        weight: directwrite_to_font_weight(weight),
        style: directwrite_to_font_style(style),
        fallbacks: None,
    };
...
fn directwrite_to_font_style(style: DWRITE_FONT_STYLE) -> FontStyle {
    match style.0 {
        0 => FontStyle::Normal,
        1 => FontStyle::Italic,
        2 => FontStyle::Oblique,
...
fn directwrite_to_font_weight(weight: DWRITE_FONT_WEIGHT) -> FontWeight {
    FontWeight(weight.0 as f32)
}
...
mod tests {
    ...
    use crate::backend::direct_write::ClusterAnalyzer;
    use crate::{
        DevicePixels, DirectXDevices, Font, PlatformTextSystem, RenderGlyphParams, Rgba, bounds,
        point, px, size,
    };
    use anyhow::Result;
    use std::ffi::c_void;
