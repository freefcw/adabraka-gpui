...
fn convert_image_to_png_format(bytes: &[u8], image_format: ImageFormat) -> Result<Vec<u8>> {
    let image = image::load_from_memory_with_format(bytes, image_format_to_image(image_format))?;
...
fn read_image_for_type(format_number: u32, format: ImageFormat) -> Option<ClipboardEntry> {
    let bytes = with_clipboard_data(format_number, |data_ptr, size| unsafe {
        std::slice::from_raw_parts(data_ptr as *mut u8 as _, size).to_vec()
    })?;
    Some(ClipboardEntry::Image(Image::new(format, bytes)))
}
...
fn image_format_to_image(value: ImageFormat) -> image::ImageFormat {
    match value {
