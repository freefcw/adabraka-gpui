...
impl WindowsDisplay {
    pub(crate) fn new(display_id: DisplayId) -> Option<Self> {
        let screen = HMONITOR(display_id.index() as _);
        let info = get_monitor_info(screen).log_err()?;
        Self::from_monitor_info(screen, display_id, info)
    }
    ...
    pub fn check_given_bounds(&self, bounds: Bounds<Pixels>) -> bool {
        ...
        let center = POINT {
            x: (f32::from(center.x) * self.scale_factor) as i32,
            y: (f32::from(center.y) * self.scale_factor) as i32,
        };
        let monitor = unsafe { MonitorFromPoint(center, MONITOR_DEFAULTTONULL) };
        if monitor.is_invalid() {
            false
        } else {
            let Some(display) = WindowsDisplay::new_with_handle(monitor) else {
                return false;
            };
    ...
    pub fn displays() -> Vec<Rc<dyn PlatformDisplay>> {
        available_monitors()
            .into_iter()
            .filter_map(|handle| {
                let display_id = Self::display_id_for_monitor(handle);
                Some(Rc::new(Self::new(display_id)?) as Rc<dyn PlatformDisplay>)
            })
            .collect()
    }
